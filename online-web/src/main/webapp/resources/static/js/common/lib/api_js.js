...
function Login(){
	...
	var Authkey		=   "";
	if(Server == "0"){
		Authkey = "c549890c7f77daa313443b7abbbd77b76d1d6434";
	}else{
		Authkey = "955aab82d077bab942dce451af57520a37f1acef";
	}
		...
		if (wb)
		   {
		      ...
		      wb.on_fire_onusernotify(wb, "login@@"+result+"@@"+Authkey+"##"+LoginID+"##"+LoginPWD);
		   }
		   else if (typeof nexacro == "undefined")
		   {
		      // Windows nexacro Browser 
		      window.document.title = "login@@"+result+"@@"+Authkey+"##"+LoginID+"##"+LoginPWD;
		   }
		   else if (nexacro)
		   {
		      // Android nexacro Browser, macOS nexacro Browser 
		      nexacro.fireUserNotify("login@@"+result+"@@"+Authkey+"##"+LoginID+"##"+LoginPWD);
		   }
}
...
//로그인 기록 남기기
function logInfo(sServer,sKey,sId,sPw){
	if(sServer == "prod"){
		document.all.server.value = "1";
	}else{
		document.all.server.value = "0";
	}
	
	document.all.loginID.value = sId;
	document.all.loginPWD.value = sPw;
...
function  Close(){
	...
	{
		// Web Browser, iOS nexacro Browser, Android Default Web Browser, iOS Default WebBrowser 
		wb.on_fire_onusernotify(wb, "close@@");
	}
	else if (typeof nexacro == "undefined")
	{
		// Windows nexacro Browser 
...
var Server			=	document.all.server.value;
	...
	window.open("https://login.kt.com/wamui/MemSelectMemberType.do?domainNameCd=2", "통화매니저");
}
// 패스워드 찾기 확장 함수
